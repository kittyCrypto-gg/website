...
/**
 * Controls optional extra lines rendered in the faux terminal banner.
 */
export type BannerLoadOptions = Readonly<{
    serverOffline?: boolean;
}>;

/**
 * @param {BannerLoadOptions} opts - Banner rendering options.
 * @returns {Promise<void>} A banner DOM fragment appended into #banner.
 */
export async function loadBanner(opts: BannerLoadOptions = {}): Promise<void> {
    ...
    prompt.innerHTML =
        '<span class="green">kitty@kittycrypto</span><span class="blue">:~</span><span class="teal">$ neofetch</span>';
    ...
    const infoLines = [
        "HatsuneMikuOS V ${MISSING_VERSION_NUMBER}",
        "----------------------",
        "User: Kitty (she/her)",
        "Host: CRT-iMac (teal)",
        "DE: Nekomimi-kawaii",
    ...
    if (opts.serverOffline === true) {
        const span = document.createElement("span");
        span.classList.add("info-line", "info-offline");
        span.textContent =
            "[notice] kittyServer is unreachable right meow. Some features are unavailable. Please try again shortly, or email kitty@kittycrypto.gg.";
        infoBlock.appendChild(span);
        infoBlock.appendChild(document.createElement("br"));
    }
    ...
    cursor.innerHTML =
        '<span class="green">kitty@kittycrypto</span><span class="blue">:~</span><span class="teal">$ </span><span class="cursor">█</span>';
